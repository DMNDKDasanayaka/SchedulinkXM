...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['generate'])) {
    ...
    // Clear existing data in correct order to respect constraints
    $db->query("DELETE FROM duty_payments");
    $db->query("DELETE FROM allocation_logs");
    $db->query("DELETE FROM allocations");
    $db->query("DELETE FROM duty_roster");
    ...
    foreach ($examSchedule as $exam) {
        ...
        $total = $exam['p_count'] + $exam['r_count'];
        ...
        $subjectCode = extractSubjectCode($exam['subject']);
        $examId = getExamId($db, $subjectCode, $exam['date']);
        $hallId = getHallId($db, $exam['venue']);
        
        // Create allocation record if we have all required IDs
        if ($supervisorId && $examId && $hallId) {
            ...
            // Record payment for hall attendant if we have their ID
            if ($attendantId) {
                $paymentStmt = $db->prepare("INSERT INTO duty_payments 
                    (lecturer_id, allocation_id, role_type, payment_amount, payment_status) 
